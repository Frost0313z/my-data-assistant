...
**주의** — Active 상태는 [`design-system.md`](design-system.md) 규칙 1의 **"선택 상태"** 갈래다. 단일 선택이라 화면에 하나뿐이다. 다만 이 컴포넌트가 Primary를 점유하므로 **주변 액션 버튼("이 주제로 AI에게 질문", "전체 대시보드에서 보기")은 중성으로 내려야 한다.** 선택 상태와 액션이 같은 색이면 어느 쪽이 눌러야 할 것인지 구분되지 않는다.
