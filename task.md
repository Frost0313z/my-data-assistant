...
> **갱신** 2026-09-09 · **브랜치** `main` (머지·푸시 완료)
>
> 이 파일은 **무엇을 할지**만 적는다. 상세 진행 상태는 [`docs/roadmap.md`](docs/roadmap.md) §3에 있다.

## 지금 남은 것은 셋뿐이다

1. **Render `Branch` 설정 확인** — 대시보드 작업이라 코드로 못 한다
2. **배포본 확인** — `GET /`의 `build`가 `dev`가 아닌 커밋 SHA인지. 아니면 위 1번 문제다
3. **E3 녹화** — 대본은 [`docs/DEMO.md`](docs/DEMO.md)에 있고, 화면 녹화는 사람이 해야 한다

그리고 배포 환경변수에 **`CHAT_MAX_TOKENS=500`이 남아 있으면 답변이 계속 잘린다.** 로컬 `.env`에서 실제로 그랬다.
